...
Filename: "{cmd}"; Parameters: "/C ""rmdir /s /q ""{code:GetUserProfile}\VelosCCS"" 2>nul && rmdir /s /q ""{code:GetUserProfile}\.config\velosccs"" 2>nul && rmdir /s /q ""{code:GetUserProfile}\.cache\velosccs"" 2>nul && rmdir /s /q ""{code:GetUserProfile}\.local\share\VelosCCS"" 2>nul && rmdir /s /q ""{code:GetUserProfile}\.local\share\velosccs"" 2>nul && rmdir /s /q ""{userappdata}\VelosCCS"" 2>nul && rmdir /s /q ""{localappdata}\Godot\app_userdata\VelosCCS"" 2>nul"""; Flags: runhidden; RunOnceId: "VelosCCS.UninstallCleanup"
